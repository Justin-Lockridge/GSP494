...
void GameEngine::Init(HWND& hWnd, HINSTANCE& hInst, bool bWindowed)
{
	...
	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"BolsterIcon.png", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &m_bolsterIconInfo, 0, &m_bolsterIcon );
	...
	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"ThiefUnitInfo.png", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
	...
	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"WarriorUnitInfo.png", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &m_warriorHoverInfo, 0, &m_warriorHover);

	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"MinotaurUnitInfo.png", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &m_minotaurHoverInfo, 0, &m_minotaurHover);

	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"MarksmanUnitInfo.png", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &m_marksmanHoverInfo, 0, &m_marksmanHover);
	...
	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"CleaveAbilityIcon.png", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &m_cleaveAbilityIconInfo, 0, &m_cleaveAbilityIcon);

	////////////////////////////////////
	// Win Screens

	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"the_archer_by_sktchwlkr-d65d3ai.jpg", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &arch_winScreenInfo, 0, &arch_winScreen);

	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"wrath_of_the_black_mage_by_nanoxepher-d5wqtx4.jpg", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &bm_winScreenInfo, 0, &bm_winScreen);

	D3DXCreateTextureFromFileEx(m_pD3DDevice, L"knight_by_emortal982-d4rp3yg.jpg", 0, 0, 0, 0, 
		D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 
		D3DCOLOR_XRGB(255, 0, 255), &war_winScreenInfo, 0, &war_winScreen);
	// Seed rand() with time
	srand(timeGetTime());
...
void GameEngine::InitGameBoard()
{
	...
	m_unit[4][15].addUnit(GOLDMINES, PLAYERTWO);
	m_unit[4][15].setWhoUnitBelongsTo(PLAYERTWO);
	m_unit[4][15].setType(GOLDMINES);
...
void GameEngine::Update(float dt)
{
		...
		if(!player1selected)
		{
			for(auto &Buttons: characterButtons)
			{
				if(Buttons.getPosition().x > -109.0f && Buttons.getPosition().x < 100.0f)
					Buttons.setPosition( (Buttons.getPosition().x + (300 * dt)), Buttons.getPosition().y);

				if(Buttons.getPosition().y > -1.0f && Buttons.getPosition().y < 300.0f)
					Buttons.setPosition( Buttons.getPosition().x , (Buttons.getPosition().y + ( 400 * dt  ) ) );

				if(Buttons.getPosition().x > 599.0f && Buttons.getPosition().x < 901.0f)
					Buttons.setPosition( (Buttons.getPosition().x - (400 * dt) ), Buttons.getPosition().y);
	...
	case BATTLE:
		...
		if(m_gamePhase == PLAYERONE_PLAYPHASE || m_gamePhase == PLAYERTWO_PLAYPHASE)
		{
		...
		if(m_player[0].checkIfActivePlayer())
		{
			...
			for(auto& Buttons: map_grid_row1) 
			{
				if(selectedUnit == 3)
				{
					if(Buttons.isOn(cursor.x, cursor.y, 3) && Buttons.col < 7)
					{
						Buttons.setColor(D3DCOLOR_ARGB(255, 255, 255, 0));
						Buttons.setHighlight(true);
					}
				...
				else
				{
					...
					if(Buttons.isOn(cursor.x, cursor.y, 3) && Buttons.col < 6)
					{
						Buttons.setColor(D3DCOLOR_ARGB(255, 255, 255, 0));
						Buttons.setHighlight(true);
					}
				...
				if(selectedUnit == 4 || selectedUnit == 5)
				{
					if(Buttons.isOn(cursor.x, cursor.y, 3) )
					{
						Buttons.setColor(D3DCOLOR_ARGB(255, 255, 255, 0));
				...
				if(Buttons.isOn(cursor.x, cursor.y, 3) && (mouseState.rgbButtons[0]& 0x80) != 0 && !keyIsDown[DIK_P])
				{
					...
					if(col == 0 && selectedUnit == 4)
						cantUseHere = true;

					if(col == 15 && selectedUnit == 4)
						cantUseHere = true;
		...
		if(m_player[1].checkIfActivePlayer())
		{
			...
			for(auto& Buttons: map_grid_row1) 
			{
				if(selectedUnit == 3)
				{
					if(Buttons.isOn(cursor.x, cursor.y, 3) && Buttons.col > 8)
					{
						Buttons.setColor(D3DCOLOR_ARGB(255, 255, 255, 0));
						Buttons.setHighlight(true);
					}
						...
						case 4: // Ability 1
							if( !m_classAbilityAnimator.getAnimationActive() )
							{
								if(m_player[1].getCurrentSpecial() >= 50)
								{
									...
									if(m_player[1].getCharacterType() == BLACKMAGE && col != 0 && col != 15) // Black hole
									{
										fmodSystem->playSound( FMOD_CHANNEL_FREE, blackHoleAbilitySFX, false, 0 );
...
void GameEngine::changePhase()
{
	...
	case PLAYERTWO_PLAYPHASE:
		...
		if(!firstTurn)
		{
			for( int i = 0; i < 5; ++i )
			{
				test = m_unit[i][15].getType();
				if( m_unit[i][15].getType() == GOLDMINES )
			...
			if(SUCCEEDED(m_pD3DSprite->Begin(D3DXSPRITE_ALPHABLEND | D3DXSPRITE_SORT_DEPTH_FRONTTOBACK)))
			{
				...
				case PLAYER1WIN:
					if(m_player[0].getCharacterType() == BLACKMAGE)
					{
						...
						D3DXMatrixScaling(&scaleMat, 0.7f, 0.7f, 0.0f);
						D3DXMatrixTranslation(&transMat, 380.0f, 300.0f, 0.0f);
						D3DXMatrixMultiply(&scaleMat, &scaleMat, &rotMat);
						D3DXMatrixMultiply(&worldMat, &scaleMat, &transMat);
						...
						m_pD3DSprite->SetTransform(&worldMat);
						//change backgroung image
						m_pD3DSprite->Draw(bm_winScreen, 0, &D3DXVECTOR3(bm_winScreenInfo.Width * 0.5f, bm_winScreenInfo.Height * 0.5f, 0.0f),
							0, D3DXCOLOR(255,255, 255,255));
					}
					...
					if(m_player[0].getCharacterType() == ARCHER)
					{
						...
						D3DXMatrixScaling(&scaleMat, 0.5f, 0.7f, 0.0f);
						D3DXMatrixTranslation(&transMat, 280.0f, 250.0f, 0.0f);
						D3DXMatrixMultiply(&scaleMat, &scaleMat, &rotMat);
						D3DXMatrixMultiply(&worldMat, &scaleMat, &transMat);

						m_pD3DSprite->SetTransform(&worldMat);
						//change backgroung image
						m_pD3DSprite->Draw(arch_winScreen, 0, &D3DXVECTOR3(arch_winScreenInfo.Width * 0.5f, arch_winScreenInfo.Height * 0.5f, 0.0f),
							0, D3DXCOLOR(255,255, 255,255));
					}
					...
					if(m_player[0].getCharacterType() ==  WARRIOR)
					{
						...
						D3DXMatrixScaling(&scaleMat, 0.85f, 0.8f, 0.0f);
						D3DXMatrixTranslation(&transMat, 380.0f, 240.0f, 0.0f);
						D3DXMatrixMultiply(&scaleMat, &scaleMat, &rotMat);
						D3DXMatrixMultiply(&worldMat, &scaleMat, &transMat);

						m_pD3DSprite->SetTransform(&worldMat);
						//change backgroung image
						m_pD3DSprite->Draw(war_winScreen, 0, &D3DXVECTOR3(war_winScreenInfo.Width * 0.5f, war_winScreenInfo.Height * 0.5f, 0.0f),
							0, D3DXCOLOR(255,255, 255,255));
					}
					break;
				case PLAYER2WIN:
...
void GameEngine::Shutdown()
{
	...
	SAFE_RELEASE(m_cleaveAbilityIcon);
	SAFE_RELEASE(m_bolsterIcon);
	// Unit Hover
	...
	SAFE_RELEASE(m_warriorHover);
	SAFE_RELEASE(m_minotaurHover);
	SAFE_RELEASE(m_marksmanHover);
	...
	//Win screens
	SAFE_RELEASE(bm_winScreen);
	SAFE_RELEASE(war_winScreen);
	SAFE_RELEASE(arch_winScreen);
...
void GameEngine::drawUIText(float dt)
{
	...
	{
		RECT none;
		wchar_t noneBuffer[64];
		GetClientRect(m_hWnd, &none);
		none.top = 277;
		none.left = 300;
		swprintf_s(noneBuffer, 64, L"You Can't Use That There!");
		m_pD3DFont->DrawText(0, noneBuffer, -1, &none, DT_TOP|DT_LEFT|DT_NOCLIP, D3DCOLOR_ARGB(255, 255, 0, 0));
	...
	if(noSpecial)
	{
		...
		GetClientRect(m_hWnd, &special);
		special.top = 277;
		special.left = 290;
		swprintf_s(specialBuffer, 64, L"Not enough special!");
		m_pD3DFont->DrawText(0, specialBuffer, -1, &special, DT_TOP|DT_LEFT|DT_NOCLIP, D3DCOLOR_ARGB(255, 255, 0, 0));
...
void GameEngine::drawIcons(int thisButton , Character thisPlayer, D3DXVECTOR3 position,D3DCOLOR a_color)
{
	...
	if(thisPlayer.getPlayerNumber() == 0 && thisPlayer.characterType == WARRIOR)
	{
		...
		if(thisButton == WARRIORABILITY1)
		{
			...
			m_pD3DSprite->Draw(m_cleaveAbilityIcon, 0, &D3DXVECTOR3(m_cleaveAbilityIconInfo.Width * 0.5f, m_cleaveAbilityIconInfo.Height * 0.5f, 0.0f),
				0, a_color);
		}
		...
		if(thisButton == WARRIORABILITY2)
		{
			...
			m_pD3DSprite->Draw(m_bolsterIcon, 0, &D3DXVECTOR3(m_bolsterIconInfo.Width * 0.5f, m_bolsterIconInfo.Height * 0.5f, 0.0f),
				0, a_color);
		}
	}
	...
	if(thisPlayer.getPlayerNumber() == 1 && thisPlayer.characterType == WARRIOR)
	{
		...
		if(thisUnit == 0)
		{
			if(thisButton == WARRIORBUTTON)
			{
				...
				m_pD3DSprite->Draw(m_warriorHover, 0, &D3DXVECTOR3(m_warriorHoverInfo.Width * 0.5f, m_warriorHoverInfo.Height * 0.5f, 0.0f),
					0, a_color);
			}
		}
		if(thisUnit == 2)
		{
			if(thisButton == MINOTAURBUTTON)
			{
				...
				m_pD3DSprite->Draw(m_minotaurHover, 0, &D3DXVECTOR3(m_minotaurHoverInfo.Width * 0.5f, m_minotaurHoverInfo.Height * 0.5f, 0.0f),
					0, a_color);
			}
		}
		if(thisUnit == 1)
		{
			if(thisButton == MARKSMANBUTTON)
			{
				...
				m_pD3DSprite->Draw(m_marksmanHover, 0, &D3DXVECTOR3(m_marksmanHoverInfo.Width * 0.5f, m_marksmanHoverInfo.Height * 0.5f, 0.0f),
					0, a_color);
			}
		}
		if(thisUnit == 3)
